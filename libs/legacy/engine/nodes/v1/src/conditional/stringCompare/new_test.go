...
func TestNew(t *testing.T) {
	t.Run("successfully creates new node", func(t *testing.T) {
		...
		// Assert the type of 'n' to be *StringCompareConditionalNode
		conditionalNode, ok := n.(*StringCompareConditionalNode)
		if !ok {
			t.Errorf("New() returned incorrect type, expected *StringCompareConditionalNode")
		}
