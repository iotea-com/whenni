...
func (n *BooleanConditionalNode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
	...
	// Ensure the condition is "equal to"
	for i, condition := range n.config.Conditions {
		if condition.Operator != "equal-to" {
			return node.Error{
				Type:   node.ValidationError,
				Reason: fmt.Sprintf("condition %d must be 'equal to'", i+1),
			}
		}
	}
...
func (n *BooleanConditionalNode) setupObservability(obsv node.Observability) error {
...
func (n *BooleanConditionalNode) setConfig(config []byte) error {
