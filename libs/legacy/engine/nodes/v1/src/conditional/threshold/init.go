...
func (n *ThresholdConditionalNode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *ThresholdConditionalNode) setupObservability(obsv node.Observability) error {
...
func (n *ThresholdConditionalNode) setConfig(config []byte) error {
