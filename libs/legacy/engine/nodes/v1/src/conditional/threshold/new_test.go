...
func TestNew(t *testing.T) {
	t.Run("successfully creates new node", func(t *testing.T) {
		...
		// Assert the type of 'n' to be *ThresholdConditionalNode
		conditionalNode, ok := n.(*ThresholdConditionalNode)
		if !ok {
			t.Errorf("New() returned incorrect type, expected *ThresholdConditionalNode")
		}
