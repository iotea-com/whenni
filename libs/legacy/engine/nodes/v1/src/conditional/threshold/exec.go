...
func (n *ThresholdConditionalNode) Exec(params node.ExecParams) node.Error {
Loop:
	for {
		select {
...
func (n *ThresholdConditionalNode) handle(ioData node.IoData) (*node.IoData, bool, bool, error) {
	...
	for i, condition := range n.config.Conditions {
		if failureReason := n.config.Model.EvaluateCondition(input, condition); failureReason == nil {
