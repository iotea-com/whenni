...
func TestNew(t *testing.T) {
	t.Run("successfully creates new message queue node", func(t *testing.T) {
		...
		// Assert the type of 'n' to be *MessageQueueSourceNode
		messageQueueNode, ok := n.(*MessageQueueSourceNode)
		if !ok {
			t.Errorf("New() returned incorrect type, expected *MessageQueueSourceNode")
		}
		...
		if len(messageQueueNode.GetOutputChans(context.Background())) != 1 || messageQueueNode.GetOutputChans(context.Background())[0].Type[0] != node.BytesDataType {
