...
func (n *AwsSNSActionSubnode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *AwsSNSActionSubnode) setConfig(config []byte) error {
...
func (n *AwsSNSActionSubnode) setupObservability(obsv node.Observability) error {
