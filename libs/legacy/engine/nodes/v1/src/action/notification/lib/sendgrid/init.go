...
func (n *SendgridActionSubnode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *SendgridActionSubnode) setConfig(config []byte) error {
...
func (n *SendgridActionSubnode) setupObservability(obsv node.Observability) error {
