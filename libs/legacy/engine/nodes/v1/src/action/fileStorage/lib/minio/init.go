...
func (n *MinioActionSubnode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *MinioActionSubnode) setupObservability(obsv node.Observability) error {
...
func (n *MinioActionSubnode) setConfig(config []byte) error {
	...
	// Validate model input attributes
