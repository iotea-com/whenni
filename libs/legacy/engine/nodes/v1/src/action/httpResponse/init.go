...
func (n *HttpResponseActionNode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *HttpResponseActionNode) setConfig(config []byte) error {
...
func (n *HttpResponseActionNode) setupObservability(obsv node.Observability) error {
	n.tracer = obsv.TracerProvider.Tracer("HttpResponseActionNodeTracer")
	n.meter = obsv.MeterProvider.Meter("HttpResponseActionNodeMeter")
