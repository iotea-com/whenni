...
func (n *TransformProcessingNode) Init(params node.InitParams) node.Error {
	startTime := time.Now()
...
func (n *TransformProcessingNode) setupObservability(obsv node.Observability) error {
...
func (n *TransformProcessingNode) setConfig(config []byte) error {
	...
	// Validate model input & output attributes
	if err := n.config.ModelInput.Validate(); err != nil {
		return fmt.Errorf("invalid model input attributes: %v", err)
	}

	if err := n.config.ModelOutput.Validate(); err != nil {
		return fmt.Errorf("invalid model output attributes: %v", err)
	}
