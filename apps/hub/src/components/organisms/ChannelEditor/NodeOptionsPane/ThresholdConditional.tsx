...
import { ChannelNode, ThresholdConditionalNodeConfig } from '@gruent/libs/engine/nodes/v1'
import { Condition, Operator } from '@gruent/libs/engine/nodes/v1/src/conditional/threshold'
...
const ThresholdConditionalOptions: FC<Props> = ({ models, orgId, spaceId }) => {
  const upsertNode = useChannelEditorStore((state) => state.upsertNode)
  const currentNode = useChannelEditorStore(
    (state) => state.currentNode as ChannelNode<ThresholdConditionalNodeConfig>,
  )
...
const ThresholdNodeModal: FC<ThresholdModalProps> = ({
  conditions,
  model,
  orgId,
  spaceId,
  ...
  const flattenSchemaFields = useCallback(
    (
      ...
      return fieldsInObject.flatMap(([attributeId, attribute]) => {
        ...
        // Only include number fields since we're doing numerical comparisons
  ...
  const handleConditionValueChange = (e: ChangeEvent<HTMLInputElement>, conditionIndex: number) => {
  ...
  if (!model) {
    return (
      <Modal id="threshold" showAccept={false}>
    ...
    <Modal id="threshold" showAccept={false}>
      <div className="overflow-scroll">
        {availableAttributes.length <= 0 && (
          <p>
            No numeric attributes were found in the '{model.name}' model.{' '}
            <a
              className="text-blue-500 underline"
              href={`/organizations/${orgId}/spaces/${spaceId}/models/${model.id}`}
            >
              Add at least one numeric field
            </a>{' '}
            and then try again.
          </p>
        )}
                ...
                >
                  ...
                  <div className="border border-green-500 rounded-sm px-6 py-3">
                    ...
                    <FormFieldSelect
                      ...
                      label="Operator"
                      options={[
                        { label: '==', value: Operator.EqualTo },
                        { label: '!=', value: Operator.NotEqualTo },
                        { label: '>', value: Operator.GreaterThan },
                        { label: '<', value: Operator.LessThan },
                        { label: '>=', value: Operator.GreaterThanEqualTo },
                        { label: '<=', value: Operator.LessThanEqualTo },
                      ]}
...
export default ThresholdConditionalOptions
